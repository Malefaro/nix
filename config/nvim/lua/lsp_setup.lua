...
lsp_installer.on_server_ready(function(server)
    ...
    if server.name == "gopls" then
        ...
        opts.on_attach = function(client, bufnr)
            ...
            map('n', '<leader>td', '<cmd>lua require("dap-go").debug_test()<CR>', map_opts)
        end
    ...
    if server.name == "rust_analyzer" then
        ...
        opts.on_attach = function(client, bufnr)
            orig_on_attach(client, bufnr)
            local map_opts = { noremap = true, silent = true }
            map('n', '<leader>ra', '<cmd>lua require"rust-tools.hover_actions".hover_actions()<CR>', map_opts)
            map('n', '<leader>rd', '<cmd>lua require("rust-tools.debuggables").debuggables({auto_focus=true})<CR>',
                map_opts)
            map('n', '<leader>rr', '<cmd>lua require("rust-tools.runnables").runnables({auto_focus=true})<CR>',
                map_opts)
            map('n', '<leader>re', '<cmd>lua require("rust-tools.expand_macro").expand_macro({auto_focus=true})<CR>',
                map_opts)
        end
